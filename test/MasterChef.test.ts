...
describe("MasterChef", () => {
    ...
    it("Deposit & Withdraw", async () => {
        ...
        const totalDeposit = await chef.stakedBalanceOfUser(POOL_ID, alice.address)
        console.info(`withdraw: all, ${toEther(totalDeposit)}`)
        await chef.connect(alice).withdraw(POOL_ID, totalDeposit)
        pendingReward = await chef.pendingReward(POOL_ID, alice.address)
        console.info(`pendingReward: ${toEther(pendingReward)}`)
    ...
    it("Deposit + Harvest", async () => {
        ...
        await chef.connect(alice).withdraw(POOL_ID, 0)
        ...
        await mineTime(3600)
        pendingReward = await chef.pendingReward(POOL_ID, alice.address)
        expect(pendingReward, `pendingReward (after 1h): ${toEther(pendingReward)}`).to.gte(ether(2700))
        expect(pendingReward, `pendingReward (after 1h): ${toEther(pendingReward)}`).to.lt(ether(2710))
    })
    ...
    it("Two deposits + Harvest", async () => {
        ...
        expect(rewardAlice, `alice reward (after 1h): ${toEther(rewardAlice)} < 910`).to.lt(ether(910))
